...
class ProductManager {
    private $pdo;
    private $jsonFile = '../produk/products.json';
    ...
    public function getProductsByCategory($category) {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM products WHERE category = :category ORDER BY created_at DESC");
    ...
    public function getProductStats() {
        try {
            ...
            // Total value
            $stmt = $this->pdo->prepare("SELECT SUM(price * stock) as total_value FROM products");
            $stmt->execute();
            $stats['total_value'] = $stmt->fetch()['total_value'] ?? 0;
            
            // Categories count
            $stmt = $this->pdo->prepare("SELECT category, COUNT(*) as count FROM products GROUP BY category");
            $stmt->execute();
    ...
    private function saveToJson($product) {
        $products = loadProducts();
        $products[] = $product;
        file_put_contents($this->jsonFile, json_encode($products, JSON_PRETTY_PRINT));
    }
    
    private function updateInJson($updatedProduct) {
        $products = loadProducts();
        foreach ($products as &$product) {
    ...
    private function syncJsonToDatabase($jsonProducts) {
